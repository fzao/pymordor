...
def getdimmodele(handlemodele):
    ...
    # return values in standard types
    return nmailles_c.value, nsortietransfert_c.value, nbuffer_c.value, \
        dt_c.value

def initmodele(handlemodele, ptr_tm_etatzero, etatproductionzero, dimep1, \
    dimep2, etattransfertzero, dimet1, dimet2, bufferproductionzero, \
    dimbp1, dimbp2):
    """ State initialization of model Handle """
    # Data preparation in ctype format
    handlemodele_c = c_int(handlemodele)
    ptr_tm_etatzero_c = (c_double * 9)(*ptr_tm_etatzero)
    etatproductionzero = \
    ...
    # calling the library
    valeur = MY_LIBRARY.InitModele(handlemodele_c, byref(ptr_tm_etatzero_c), \
...
def runmordor(handlemodele, ptr_tm_fin):
    """ Run the spatialized Modor for the model Handle """
    # Data preparation in ctype format
    handlemodele_c = c_int(handlemodele)
    ptr_tm_fin_c = (c_double * 9)(*ptr_tm_fin)
    
    # calling the library
    valeur = MY_LIBRARY.RunMordor(handlemodele_c, byref(ptr_tm_fin_c))
